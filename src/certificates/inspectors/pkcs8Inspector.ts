...
export function inspectPkcs8(
    ...
    const encrypted = detectEncryption(bytes, encoding);

    if (encrypted) {
        return lockedInspection(bytes, filePath, encoding);
    }

    try {
        const privateKey = createPrivateKey(bytes, encoding);

        return unlockedInspection(privateKey, bytes, filePath, encoding);
    } catch {
        return {
            values: {
                ...
                Encoding: encoding,
                Status: "Unsupported",
                Summary: "PKCS#8 private key detected, but parsing failed. The key may be encrypted or unsupported.",
...
export function inspectPkcs8Unlocked(
    ...
    const encoding = detectEncoding(bytes);
    try {
        const privateKey = createPrivateKey(bytes, encoding, password);

        return unlockedInspection(privateKey, bytes, filePath, encoding, true);
    } catch {
        return {
            values: {
                Type: "PKCS#8 Private Key",
                ...
                Summary: "Failed to unlock PKCS#8 private key. The password may be incorrect or the key format may be unsupported.",
                ...fileMetadata(bytes, filePath)
            }
        };
    }
...
function createPrivateKey(
    ...
    if (encoding === "PEM") {
        return crypto.createPrivateKey({key, format: "pem", passphrase: password});
    }
...
function unlockedInspection(
    ...
    const algorithm = normalizeAlgorithm(privateKey.asymmetricKeyType || "");
    const keySize = details?.modulusLength ? `${details.modulusLength} bits` : "";
    const curve = details?.namedCurve || "";
    const summaryParts = [algorithm || "PKCS#8", keySize, curve, "private key"].filter(Boolean);

    return {
        values: {
            Protection: passwordEncrypted ? "Password encrypted" : "",
            Type: "PKCS#8 Private Key",
            Encoding: encoding,
            Status: "Unlocked",
            Algorithm: algorithm,
...
function lockedInspection(
    ...
    return {
        values: {
            ...
            Summary: "Encrypted PKCS#8 private key detected. Enter the password to inspect the key.",
            ...fileMetadata(bytes, filePath)
        }
    };
}
...
function detectEncryption(
    ...
    if (encoding === "PEM") {
        ...
        return text.includes("-----BEGIN ENCRYPTED PRIVATE KEY-----");
    }
