...
export function inspectSshPrivateKey(
    ...
    const algorithm = algorithmFromFilePath(filePath);
    ...
    if (!publicKey) {
        return {
            values: {
                ...
                Algorithm: algorithmFromFilePath(filePath),
...
function extractPublicKey(
    ...
    const temporaryDirectory = fs.mkdtempSync(path.join(os.tmpdir(), "sato-ssh-key-"));
...
function createUnlockedPrivateKeyCopy(
    ...
    const temporaryDirectory = fs.mkdtempSync(path.join(os.tmpdir(), "sato-ssh-unlock-"));
...
function parsePublicKey(
    ...
    const algorithm = formatAlgorithm(parts[0]);
...
function algorithmFromFilePath(
    filePath: string
): string {
